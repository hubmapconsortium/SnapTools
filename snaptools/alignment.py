...
def index_ref(input_fasta, output_prefix, path_to_aligner, aligner, num_threads):
    """
    ...
    if aligner.lower() == "bowtie2":
        subprocess.check_call(
            [path_to_aligner + "bowtie2-build", "-f", input_fasta, output_prefix]
        )
    # bowtie
    if aligner.lower() == "bowtie":
        subprocess.check_call(
            [path_to_aligner + "bowtie-build", "-f", input_fasta, output_prefix]
        )
    # bwa
    if aligner.lower() == "bwa":
        subprocess.check_call([path_to_aligner + "bwa", "index", input_fasta])
    return 0
...
def run_align_pe(
    ...
    try:
        ...
        sys.exit(
            "error: fail to run alignment, check if aligner and reference genome is correct!"
        )
    ftmp.close()
